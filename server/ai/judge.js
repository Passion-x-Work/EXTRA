...
function buildSystemPrompt(profile, fragments, mode, tone) {
  ...
  if (mode === "reverse-persuasion") {
    ...
    return [
      ...
      "\n[대사·말투 규칙]",
      `- line은 '${who}'가 플레이어(미야모토)의 반박에 보이는 반응 대사다. 반드시 '${who}'로서 1~2문장만 말한다.`,
      "- 사극체·옛말투('~하옵니다/이옵니다/하시옵/과인' 등) 절대 금지. 현대 인물이다.",
      tone === "meme"
        ? "- 밈·유행어를 적극 섞은 가벼운 인터넷 말투로 리액션 크게(예: 'ㄹㅇ', '인정', '갓겜각'). 단 판정(grade) 자체는 진지하게."
        : "- 차분하고 자연스러운 현대 게임 개발 현장의 구어체로 말한다.",
      "- 게이지·점수·승패는 언급하지 않는다. grade(등급)로만 판정한다.",
      "- grade 기준 — 탁월: 유혹을 완전히 무너뜨리는 통찰·재치가 담긴 특출난 반박(정말 뛰어날 때만 아껴서). 정합: 철학을 제대로 지킴. 부분: 방향은 맞으나 약함. 불합치/역효과: 유혹에 동조.",
